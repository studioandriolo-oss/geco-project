    ...
    if uploaded_file is not None:
        if 'ultimo_file_caricato' not in st.session_state or st.session_state.ultimo_file_caricato != uploaded_file.file_id:
            try:
                ...
                # Riparazione automatica colonne vuote per evitare crash al caricamento
                df_wbs = pd.DataFrame(dati_caricati.get('wbs', []))
                if df_wbs.empty: df_wbs = pd.DataFrame([{'ID_WBS': '1', 'Attività': 'Progetto Principale', 'BAC_Budget': 0.0, '%_Completamento': 0.0, 'AC_Costo_Reale': 0.0, 'ID_OBS_Assegnato': None, 'Predecessori': ''}])
                st.session_state.wbs_data = df_wbs
                
                df_obs = pd.DataFrame(dati_caricati.get('obs', []))
                if df_obs.empty: df_obs = pd.DataFrame(columns=['ID_OBS', 'Ruolo', 'Risorsa', 'Tipo_Contratto', 'Note'])
                st.session_state.obs_data = df_obs
                
                df_reg = pd.DataFrame(dati_caricati.get('registro', []))
                if df_reg.empty: df_reg = pd.DataFrame(columns=['Data', 'N_Doc', 'Fornitore', 'Voce_WBS', 'Importo_Netto', 'Descrizione'])
                st.session_state.registro_data = df_reg
                
                df_capa = pd.DataFrame(dati_caricati.get('capa', []))
                if df_capa.empty: df_capa = pd.DataFrame(columns=['Data_Apertura', 'ID_WBS_Rif', 'Tipo_Azione', 'Descrizione', 'Responsabile_OBS', 'Stato'])
...
with tab2:
    ...
    # Salvataggio semplice, senza required=True che bloccava le righe
...
with tab6:
    ...
    if st.button("💾 SALVA REGISTRO E AGGIORNA COSTI", type="primary", use_container_width=True):
        ...
        aggiorna_costi_reali()
        st.success("✅ Dati salvati con successo! Controlla i grafici nel Tab 5.")
...
with tab7:
    ...
    if st.button("💾 SALVA REGISTRO CAPA", type="primary", use_container_width=True):
        ...
        st.success("✅ Interventi salvati con successo!")
